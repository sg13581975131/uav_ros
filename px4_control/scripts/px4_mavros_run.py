...
class Px4Controller:
    ...
    def start(self):
        ...
        print("current_pose_x: " + str(current_pose_x) + " current_pose_y: " + str(current_pose_y))
        self.cur_target_pose = self.construct_target(current_pose_x, current_pose_y, self.takeoff_height, self.current_heading)
        for i in range(10):
        ...
        if self.takeoff_detection():
            rospy.logwarn("Vehicle Took Off!!!!")
        else:
            rospy.logerr("Vehicle Took Off Failed!")
            return
        '''
        main ROS thread
        '''
    ...
    def set_target_position_callback(self, msg):
        rospy.loginfo("Received New Position Taske!")
    ...
    def custom_activity_callback(self, msg):
        ...
        if msg.data == "HOVER":
            ...
            rospy.logerr("Received Custom Activity:", msg.data, "not supported yet!")
