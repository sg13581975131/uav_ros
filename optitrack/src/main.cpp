...
int main(int argc, char **argv)
{
   ...
   */
  ros::Publisher chatter_pub = n.advertise<optitrack::Vel_Alt>("/optitrack/Vel_Alt", 1000);
...
/* 设置地址复用许可, 根据具体情况判断是否增加此功能 */  
   ...
   time_t tt = time(NULL);
    ...
    FILE* fp;
    char fname[50];
    sprintf(fname,"optitrack-%d%d%d%d.txt",t->tm_mon + 1,t->tm_mday,t->tm_hour,t->tm_min);
   // printf("%s\n",fname);
...
//}//printf("recv ok!   ");  
 ...
 //  opt_speed.pitch=OptitrackData.UavData.pitch;
//opt_speed.yaw=OptitrackData.UavData.yaw;
//opt_speed.roll=OptitrackData.UavData.roll;
//opt_speed.qx= OptitrackData.UavData.qx;
//opt_speed.qx= OptitrackData.UavData.qy;
//opt_speed.qx= OptitrackData.UavData.qz;
//opt_speed.qx= OptitrackData.UavData.qw;
   ros::Time current = ros::Time::now();
   //double t = current.toNSec()/1e9;
double t = current.toSec()*1e9;
 write<<t <<" "<<setiosflags(ios::fixed)<<setprecision(6)<<opt_speed.X<<" "<<opt_speed.Y<<" "<<opt_speed.Z<<endl;
